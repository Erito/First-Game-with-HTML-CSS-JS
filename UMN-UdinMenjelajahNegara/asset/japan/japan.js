...
document.addEventListener("DOMContentLoaded", function () {
    ...
    button.textContent = "Rumah Kakek";
    button_container.appendChild(button);
    ...
    button.addEventListener("click", function () {
        ...
        setTimeout(() => {
            ...
            checkQuestCompletion("fotoJepang");
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    button.addEventListener("click", function () {
        ...
        setTimeout(() => {
            ...
            checkQuestCompletion("mandiJepang");
            blackScreen.style.opacity = "0"; // Fade out
            blackScreen.style.pointerEvents = "none";
        }, 1500);
    });
    ...
    foods.forEach(food => {
        ...
        foodButton.addEventListener("click", function () {
            ...
            setTimeout(() => {
                ...
                checkQuestCompletion("makanJepang");
...
//TOMBOL BUAT KEBUN
document.addEventListener("DOMContentLoaded", function () {
    let button_container = document.querySelector(".button-kebun");
    let button = document.createElement("button");
    button.className = "kebun-button";
    button.textContent = "Kebun";
    button_container.appendChild(button);
    function spawnButtonKebun() {
        const kebunX = 13;
        const kebunY = 180;
        const range = 15;
        if (Math.abs(x - kebunX) < range && Math.abs(y - kebunY) < range) {
            button.classList.remove("hidden");
        } else {
            button.classList.add("hidden");
        }
    }
    setInterval(spawnButtonKebun, 200);
    document.addEventListener("keydown", function (event) {
        if (event.key === "Enter" && !button.classList.contains("hidden")) {
            button.click();
        }
    ...
    button.addEventListener("click", function () {
        ...
        setTimeout(() => {
            ...
            checkQuestCompletion("kebunJepang");
            blackScreen.style.opacity = "0"; // Fade out
            blackScreen.style.pointerEvents = "none";
        }, 1500);
    });
